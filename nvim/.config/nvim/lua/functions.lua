...
function replaceWithProperXXX()
    local startpos = vim.fn.getpos("'<'")
    local endpos = vim.fn.getpos("'>'")
    local ft = vim.bo.filetype
    local docType = ""
    local originalLines = vim.api.nvim_buf_get_lines(vim.api.nvim_get_current_buf(), startpos[2] - 1, endpos[2], false)
    ...
    vim.api.nvim_buf_set_lines(
        ...
        endpos[2],
        false,
        { "/** replace with proper " .. docType .. " */" }
    )
end

vim.keymap.set("v", "<Leader>cv", ":lua replaceWithProperXXX()<Return>")
